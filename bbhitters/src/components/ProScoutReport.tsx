...
import { ScoutReport, SwingFlaw } from "../types";
...
export default function ProScoutReport({ report, flaws }: ProScoutReportProps) {
  if (!report) {
    return (
      <div id="scout-report-empty" className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center text-slate-500">
        ...
        <p className="text-sm font-display">Upload a swing or select a pro profile to generate the Scouting & Coaching Engine diagnostics.</p>
      </div>
    );
  }
  ...
  return (
    <div id="pro-scout-report-section" className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-2xl space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-850 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-display text-lg font-bold text-slate-50 tracking-tight">
              Pro Kinematics Scout Report
            </h2>
            <p className="text-xs text-slate-400">MLB Hitter Lab swing-efficiency diagnostic engine</p>
          </div>
        </div>

        {/* Swing Efficiency Gauge Block */}
        <div className="flex items-center gap-4 bg-slate-950 px-4 py-2.5 border border-slate-800 rounded-xl self-start md:self-auto">
          <div className="text-right">
            <div className="text-[10px] uppercase font-bold text-slate-500 tracking-wider">Swing Efficiency</div>
      ...
      {/* Scout Summary & Contact Depth Block */}
        ...
        <div className="bg-slate-950 border border-slate-850 rounded-lg p-4 flex flex-col justify-between">
          <div>
            ...
            <div className={`text-lg font-display font-bold ${report.contactDepth === "Optimal" ? "text-emerald-400" : "text-amber-400"}`}>
              {report.contactDepth} (vs Front Foot)
            </div>
          </div>
          <div className="text-xs text-slate-400 mt-2">
            {report.contactDepth === "Optimal" 
              ? "Impact occurred exactly 2.5 inches out front of the lead toe box, guaranteeing maximum torque release of the bat speed."
              : "Impact occurred too far forward/deep, which leads to early rollover casting or barrel lagging."}
          </div>
          <div className="border-t border-slate-850 pt-2 mt-3 flex justify-between items-center text-[10px] text-slate-500 font-mono">
            <span>SQUARED-UP INDEX:</span>
        ...
        {activeFlaws.length === 0 ? (
          <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-4 flex items-center gap-3">
            ...
            <div className="text-xs text-slate-300">
              <span className="font-semibold text-emerald-400">No Flaws Detected.</span> Kinematic sequencing conforms precisely with elite professional standard timing constraints (Pelvis deceleration occurs immediately prior to barrel entry).
            </div>
          </div>
        ) : (
          <div className="space-y-3">
      ...
      <div className="space-y-3 pt-2">
        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
          <CheckCircle2 className="w-4 h-4 text-emerald-500" /> Corrective Training drills
        </h4>
